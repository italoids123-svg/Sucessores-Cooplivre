...
const LEIAME_ROWS: (string[])[] = [
  ...
  ["Cadeiras", "Cada linha vira uma posição no mapa. Se o Cargo já existe, a linha é ignorada — cadeiras existentes (ocupante, nível, diretoria) só mudam por pedido direto à consultoria. Se o Cargo é novo, uma cadeira é criada; preenchendo Nome, ela já entra com ocupante, senão entra como vaga. Nível precisa ser exatamente um dos já usados no mapa (C-Level, Diretoria, Gerência Executiva, Gerência ou Coordenação) para aparecer na navegação."],
